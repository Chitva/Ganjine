...
//                ViewBag.CompanyIntroductionTitle = Setting.CompanyIntroductionTitle ?? "معرفی مجموعه";

//            }
//            ViewBag.Banners = _uow.Set<Banner>().Where(x => x.LanguageId == 1).OrderByDescending(x => x.Id).ToList();

//            var q = _RService.ServiceTabs.Where(s => s.Name == "شرح محصول" && s.ServiceGroup.IsHome);
...
//            var latestNews = _rNewsRepository.News.Where(_ => _.LanguageId == 1).OrderByDescending(_ => _.CreationDate).Take(10);
...
//            //var viewpoint = _rViewpointRepository.Viewpoints.FirstOrDefault(_ => _.IsShown && _.LanguageID == 1);
//            //if (viewpoint != null)
//            //{
//            //    viewPointVM.Id = viewpoint.Id;
//            //    viewPointVM.LanguageID = 1;
...
//            #region LinkBox
//            //var links = _uow.Set<LinkBox>().Where(_ => _.LanguageId == 1 && _.IsShown).ToList();
//            //ViewBag.Links = links;
//            #endregion
