function AddMenuSubMenu(e) {
    ...
    $.ajax({
        url: "/Admin/Product/HasTab",
        data: "{ 'id': '" + $("#HiddenMenuId").val() + "'}",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        type: "POST",
        async: false,
        success: function (e) { HasTab = e.Count }
    });
    if (HasTab == 0 || e == "Menu") {
        $("#TypeOpr").val(e);
    ...
    if (e == "1") {
        $("#TypeOpr").val("edit");
        $.ajax({
            url: "/Admin/Product/GetInfoServiceGroup",
            data: "{ 'id': '" + $("#HiddenMenuId").val() + "'}",
            dataType: "json",
            contentType: "application/json; charset=utf-8",
            type: "POST",
...
function ClassSettingUpDown(e, t) {
    ...
    if (!t.hasClass("collapsable") && !t.hasClass("expandable") && (e.hasClass("lastCollapsable")
    ...
    $.ajax({
        url: "/Admin/Product/UpDownServiceGroup",
...
function WhichOprForPriority() { if ($("#ParentMenuName").val().length == 0) {
    ...
    return false;
     ...
     var t = $("#TypeOpr").val();
     if (t == "Menu") { e = null } if (t != "edit") {
         $.ajax({
             url: "/Admin/Product/GetPriority",
             data: "{ 'id': '" + e + "'}",
             dataType: "json",
             contentType: "application/json; charset=utf-8",
             type: "POST",
