...
(function (window, angular) {
    ...
    window.simon = angular.module("simon", []);
...
(function (window, angular, app) {
    ...
    app.directive("files", ["$compile", "$filter", "$parse", "$q", "$timeout",
        function ($compile, $filter, $parse, $q, $timeout) {
            ...
            var style = (function () {
                ...
                for (var i in size) {
                    for (var j in style) {
                        result += "." + i + j + "{";
                        for (var k in style[j]) {
                            result += k + ":" + (style[j][k] * size[i]) + "rem;";
                        }
                        result += "}";
                    }
                }
                return "<style class='files'>" + ".file-icon {font-family: Arial,Tahoma,sans-serif;font-weight: 300;display: inline-block;width: " + (style[".file-icon"]["width"] * size.md) + "rem;height: " + (style[".file-icon"]["height"] * size.md) + "rem;background: #999;position: relative;border-radius: " + (style[".file-icon"]["border-radius"] * size.md) + "rem;text-align: center;-webkit-font-smoothing: antialiased;}.file-icon::before {display: block;content: '';position: absolute;top: 0;right: 0;width: 0;height: 0;border-bottom-left-radius: " + (style[".file-icon::before"]["border-bottom-left-radius"] * size.md) + "rem;border-width: " + (style[".file-icon::before"]["border-width"] * size.md) + "rem;border-style: solid;border-color: #FFF #FFF rgba(255,255,255,.35) rgba(255,255,255,.35);}.file-icon::after {display: block;content: attr(data-type);position: absolute;bottom: 0;left: 0;font-size: " + (style[".file-icon::after"]["font-size"] * size.md) + "rem;color: #fff;text-transform: uppercase;width: 100%;padding: 0; padding-top: " + (style[".file-icon::after"]["padding-top"] * size.md) + "rem; padding-bottom: " + (style[".file-icon::after"]["padding-bottom"] * size.md) + "rem; white-space: nowrap;overflow: hidden;}" + result + "</style>";
            })();
            ...
            return {
                ...
                compile: function (element, attrs) {
                    ...
                    // create virtual button for add file
                    var addButton = angular.element("<button type='button' hidden></button>");
                    ...
                    return function (scope, element, attrs) {
                        ...
                        scope.options = angular.extend(
                                            {
                                                autoUpload: true,
                                                colors: ["#f44336", "#009688", "#673ab7", "#ff9100", "#00bcd4", "#4caf50", "#76ff03", "#ffca28", "#2196f3", "#e91e63", "#d500f9"],
                                                getColor: function (file) {
                                                    ...
                                                    // file key
                                                    var key = file.ext || file.name || "";

                                                    // make result
                                                    return this.colors[parseInt(key.split('').map(function (i) { return i.charCodeAt(0) }).join("")) % this.colors.length];
                                                }
                                            },
                                            scope.options || {}
                                        );

                        // set color to all available files
                        scope.files.map(function (file) {

                            // set
                            file.color = scope.options.getColor(file);
                        });

                        // check available scope attribute
                        if (attrs.scope) {

                            // check available scope in parent
                            ...
                            // bind current scope to parent
                            scope.scope = scope;
                        }

                        // new file uploader
                        var r = new Resumable(scope.options);

                        // assign browse button
                        r.assignBrowse(addButton);

                        // when select many files
                        r.on('filesAdded', function (files) {
                            ...
                            files.map(function (file) {
                                ...
                                var model = {
                                    ...
                                    type: file.file.type,
                                    get color() {
                                        return scope.options.getColor(this);
                                    },
                                    get ext() {
                                        return /[^.]+$/.exec(file.fileName)[0].toLowerCase();
                                    },
                                    isComplete: function () {
                                        return this.file.isComplete() && !this.isError() && !this.isPaused();
                                    },
                                    isError: function () {
                                        return false;
                                    },
                                    isPaused: function () {
                                        return this.file.isPaused();
                                    },
                                    isPending: function () {
                                        return !this.isComplete() && !this.isError() && !this.file.isPaused() && !this.file.isUploading();
                                    },
                                    isUploading: function () {
                                        return this.file.isUploading() && !this.isPaused();
                                    }
                                };
                                ...
                                // apply to angular model
                                $timeout(function () {

                                    // add file to repository
                                    scope.files[scope.files.length] = model;
                                });
                                ...
                                if (file.file.type.indexOf("image/") == 0) {
                                    ...
                                    fileReader.onload = function (event) {
                                        ...
                                        // set available data
                                        $timeout(function () { model.data = event.target.result; });
                                    }
                                }
                            });
                        ...
                        r.on('fileProgress', function (file) {
                            ...
                            $timeout(function () {
                                ...
                                // calc percent
                                file.model.percent = file.progress() * 100;
                            });
                        });
                        ...
                        r.on('fileError', function (file) {
                            ...
                            $timeout(function () {
                                ...
                                // make status
                                file.model.isError = function () {

                                    // make result
                                    return !file.model.isUploading() && !file.model.isPaused();
                                };
                            });
                        });

                        // calc complete status
                        scope.isComplete = function () {
                            ...
                            for (var i = 0; i < r.files.length; i++) {
                                ...
                                // check each file
                                if (!r.files[i].model.isComplete()) return false;
                            }

                            return true;
                        }

                        // calc uploading status
                        scope.isUploading = function () {
                            ...
                            for (var i = 0; i < r.files.length; i++) {
                                ...
                                // check each file
                                if (r.files[i].model.isUploading()) return true;
                            }
                        ...
                        scope.isError = function () {
                            ...
                            for (var i = 0; i < r.files.length; i++) {
                                ...
                                // check each file
                                if (r.files[i].model.isError()) return true;
                            }
                        ...
                        scope.add = function () {
                            ...
                            // click to virtual browse button
                        ...
                        scope.clear = function () {
                            ...
                            // get clear function
                            var fn = scope.options.on && scope.options.on.clear ? scope.options.on.clear() : null;

                            // wait to callback function
                            $q.when(fn)
                                .then(
                                    function (result) {

                                        // check to prevent clear
                                        if (result == false) return;

                                        // cancel and remove all files
                                        r.cancel();
                        ...
                        scope.remove = function (file) {
                            ...
                            // get remove function
                            var fn = scope.options.on && scope.options.on.remove ? scope.options.on.remove() : null;
                            ...
                            $q.when(fn)
                                .then(
                                    function (result) {
                                        ...
                                        // check to prevent remove
                                        if (result == false) return;

                                        // find cancel function
                                        if (file.file && file.file.cancel) {

                                            // cancel and remove current file
                                            file.file.cancel();
                                        }
                        ...
                        scope.upload = function () {
                            ...
                            r.files.map(function (file) {
                                ...
                                // check has error
                                if (file.model.isError()) {

                                    // try to upload
                                    scope.retry(file.model);
                                }
                            });
                        ...
                        if (scope.options.on) {
                            ...
                            if (scope.options.on.complete) {
                                ...
                                r.on('complete', function () {
                                    ...
                                    // prevent to extera call
                                    if (Date.now() - lastFireTime < 250) return;

                                    // update time
                                    lastFireTime = Date.now();

                                    // fire function
                                    scope.options.on.complete(scope.files);
                                });
                            }

                            // bind events
                            for (var i in scope.options.on) {

                                // check allow event
                                if (["clean", "complete", "remove"].indexOf(i) == -1) {

                                    // bind event
                                    r.on(i, scope.options.on[i]);
                                }
                            }
                        }
                        ...
                        // compile element
                        element.replaceWith($compile(result)(scope));
                    }
                }
            }
        }
...
})(window, window.angular, window.simon);